...
import { ExpandablePurchaseHistoryList } from '../components/ExpandablePurchaseHistoryList';
...
const PurchaseHistoryScreen = ({ navigation }) => {
    useEffect(async () => {
        AsyncStorage.getItem('id').then(value => {
            ...
            GetPurchasehistory(value, currentDate)
        });
    }, []);

    const GetPurchasehistory = (value, dateValue) => {
        //console.log(`http://bustle.ticketplanet.ng/GetPurchaseHistoryWeek/${value}/${dateValue}`)
        setLoading(false);
        fetch(`http://bustle.ticketplanet.ng/GetPurchaseHistoryWeek/${value}/${dateValue}`, {
            method: 'Get',
            headers: {
        ...
        <SafeAreaView style={{ flex: 1 }}>
            <View style={styles.container}>
                ...
                <DatePicker
                    ...
                    onDateChange={(date) => {
                        setDate(date);
                        GetPurchasehistory(userId, date)
                    }}
                />
